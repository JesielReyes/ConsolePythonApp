...
>
  {spendingChartData.map((_, index) => (
    ...
    <section className="activity-preview"><div className="section-heading"><div><p className="eyebrow">LATEST ACTIVITY</p><h2>Recent transactions</h2></div><a href={`/account/${accounts[0]?.accountNumber ?? '8472'}`}>View all</a></div><TransactionList transactions={transactions.slice(0, 3)} onBalanceUpdated={updateAccountBalance} /></section>
    {selectedAccount && <AccountDetailsModal account={selectedAccount} transactions={transactions} onClose={() => setSelectedAccount(null)} onBalanceUpdated={updateAccountBalance} onWithdraw={(account) => navigate(`/account/${account.accountNumber}?withdraw=true`)} />}
