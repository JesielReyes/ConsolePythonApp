...
type Props = { account: Account; transactions: Transaction[]; onClose: () => void; onWithdraw?: (account: Account) => void; admin?: boolean }

export function AccountDetailsModal({ account, transactions, onClose, onWithdraw, admin = false }: Props) {
  ...
  return <div className="modal-backdrop" role="presentation" onClick={onClose}><section className="account-modal" role="dialog" aria-modal="true" aria-labelledby="account-modal-title" onClick={(event) => event.stopPropagation()}>
    <div className="modal-header"><div><p className="eyebrow">{account.accountType.toUpperCase()} ACCOUNT</p><h2 id="account-modal-title">•••• {account.accountNumber}</h2></div><button className="icon-button" type="button" aria-label="Close account details" onClick={onClose}><X size={21} /></button></div>
    ...
    <div className="modal-section-heading"><h3>Recent transactions</h3><span>{accountTransactions.length} shown</span></div><TransactionList transactions={accountTransactions} />
